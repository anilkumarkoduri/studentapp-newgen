...
//        data: [{id:1, name:'Root', parent: 0}],
    	data: [{id:1, code:'Root', name:'Root', parent: 0}],
        showControls: false,
        allowEdit: false,
        onAddNode: null,
        onDeleteNode: null,
    ...
    function OrgChart($container, opts) {
        ...
        this.draw = function() {
            ...
            $container.find('.org-add-button').click(function(){
                if(self.opts.onClickNode !== null){
                    self.opts.onClickNode(nodes[$(this).attr('node-id')]);
                }
            });
            
            $container.find('.org-add').dblclick(function(){
                if(self.opts.onDblClickNode !== null){
                    self.opts.onDblClickNode(nodes[$(this).attr('node-id')]);
                }
            });
...
//            self.addNode({id: nextId, name: '', parent: parentId});
            self.addNode({id:nextId, code:'', name:'NEW', parent: parentId});
        }
    ...
    function Node(data) {
        ...
        this.formatNode = function(opts) {
            ...
            if(typeof data.name !== 'undefined') {
                nameString = "<p class='org-add' style='cursor: pointer' node-id='"+this.data.id+"'><strong><u>"+self.data.name+"</u></strong></p>";
            }
            if(typeof data.code !== 'undefined') {
                descString = '<p>'+self.data.code+'</p>';
            }
            
            if(opts.showControls) {
//                var buttonsHtml = "<div class='org-add-button'>"+opts.newNodeText+"</div><div class='org-del-button'></div>";

                if(self.data.tableUid == 'subInv') {
                    var buttonsHtml = "<div class='org-add-button'></div><div></div>";
                } else {
                    var buttonsHtml = "<div class='org-add-button' node-id='"+this.data.id+"'>"+opts.newNodeText+"</div><div></div>";
                }
            }
            else {
                buttonsHtml = '';
